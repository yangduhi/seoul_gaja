# Phase 03 Acceptance Contract — Core Map and List

## PASS Conditions


- initial application data uses one snapshot request.
- 121 official places remain searchable even when data is unavailable.
- map and list share selection and filter state.
- map failure leaves search, filter, list, and selection operational.
- geolocation is requested only by explicit user action and no coordinates reach network/storage.
- mockup 01 and 05 information hierarchy is present.
...
EVIDENCE: docs/evidence/phase-03/phase-receipt.json
BLOCKERS: none or exact blockers
NEXT_ALLOWED_PHASE: 04 or none
```
