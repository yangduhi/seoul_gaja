...
- UI/API는 `sourceUpdatedAt`과 `fetchedAt`을 쓰며 D1 column은 저장소 스키마의 snake_case 이름(`source_updated_at`, `fetched_at`)을 따른다. design token authority는 `design/design-tokens.json`이다.
