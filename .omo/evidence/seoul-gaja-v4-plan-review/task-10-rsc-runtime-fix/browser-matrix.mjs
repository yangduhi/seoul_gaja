...
const evidenceDir = resolve(root, `.omo/evidence/seoul-gaja-v4-plan-review/task-10-rsc-runtime-fix/${phase}`);
const candidate = {
...
const server = spawn(process.execPath, [resolve(root, "node_modules/vinext/dist/cli.js"), "dev", "--host", "localhost", "--port", String(port)], {
  cwd: root,
  env: { ...process.env, WRANGLER_LOG_PATH: `.wrangler/task-10-rsc-runtime-fix-${phase}.log` },
  windowsHide: true,
});
...
async function runReadyJourney(browser, pass, viewport) {
  ...
  await page.goBack({ waitUntil: "domcontentloaded", timeout: 10_000 });
  await page.waitForFunction(() => window.location.pathname === "/" && document.querySelector("[data-detail-surface]") === null);
  const directBrowserBack = await snapshot(page);
  await page.screenshot({ path: resolve(evidenceDir, `pass-${pass}-${viewport.width}x${viewport.height}-direct-browser-back.png`), fullPage: true });
  await context.close();
  return { pass, viewport, expectedSurface, beforeListOpen, listOpened, listReplay, nextActionBox: nextBox, nextOpened, nextReplay, directBrowserBack, ...observed };
}

async function runDirectAndInvalid(browser, pass, viewport) {
  ...
  await page.goto(`${origin}/places/does-not-exist?visualFixture=ready-v1`, { waitUntil: "networkidle" });
  ...
  await page.screenshot({ path: resolve(evidenceDir, `pass-${pass}-invalid-${viewport.width}x${viewport.height}.png`), fullPage: true });
  await page.waitForLoadState("networkidle");
  await context.close();
  ...
  return { pass, viewport, direct, reload, invalid, invalidShareAbsent, ...observed };
}

async function runMapRetry(browser, pass, viewport) {
  const context = await browser.newContext({ viewport });
  const page = await context.newPage();
  const observed = observe(page);
  ...
  await page.screenshot({ path: resolve(evidenceDir, `pass-${pass}-map-retry-${viewport.width}x${viewport.height}.png`), fullPage: true });
  await page.waitForLoadState("networkidle");
  await context.close();
  if (!hitTarget) throw new Error(`${viewport.width}x${viewport.height}: map retry does not own its center hit point.`);
  return { pass, viewport, hitTarget, announcement: "다시 시도했습니다.", ...observed };
}
...
try {
  ...
  const mapRetry = [];
  for (const pass of [1, 2]) for (const viewport of viewports) {
    directAndInvalid.push(await runDirectAndInvalid(browser, pass, viewport));
    mapRetry.push(await runMapRetry(browser, pass, viewport));
  }
