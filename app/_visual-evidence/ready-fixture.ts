...
import { parseCatalog, parseForecast, parseHistory, parseSnapshot } from "../../server/product-read-model-parsers";
...
function buildReadyVisualFixture(): ProductViewModel {
  ...
  const history = parseHistory(readyData.history, catalog.places);
  if (history.status !== "READY") throw new VisualFixtureError("history");
  return { catalog: catalog.places, snapshot: snapshot.view, officialForecast: forecast.view, history: history.view };
}
