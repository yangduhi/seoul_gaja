...
let armedCatalogReplayNavigation: ((...args: readonly unknown[]) => unknown) | null = null;
...
function armCatalogReplayNavigationGuard(): void { if (armedCatalogReplayNavigation !== null) return; const navigate = Reflect.get(window, vinextNavigateKey); if (typeof navigate !== "function") return; armedCatalogReplayNavigation = navigate; Reflect.set(window, vinextNavigateKey, (...args: readonly unknown[]) => { const original = armedCatalogReplayNavigation; armedCatalogReplayNavigation = null; if (original === null) return Promise.resolve(); Reflect.set(window, vinextNavigateKey, original); if (isCatalogHistorySentinel(window.history.state)) return Promise.resolve(); return Reflect.apply(original, window, args); }); }
...
export function closeInAppPlaceDetail(): boolean { if (window.history.state?.entry !== "sheet") return false; const returnPath = readRestoreContext()?.returnPath ?? "/"; window.history.replaceState({ entry: "catalog-replay" }, "", returnPath); armCatalogReplayNavigationGuard(); window.dispatchEvent(new CustomEvent(closeEvent)); return true; }
