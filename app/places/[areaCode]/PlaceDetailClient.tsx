...
const vinextNavigateKey = "__VINEXT_RSC_NAVIGATE__";
const safeAreaCode = /^[A-Za-z0-9_-]+$/;
let guardedCatalogReplayNavigation: ((...args: readonly unknown[]) => unknown) | null = null;
...
export function ensureCatalogHistorySentinel(): void { if (window.location.pathname !== "/" || window.history.state?.entry === "catalog-root") return; window.history.replaceState({ entry: "catalog-root" }, "", `${window.location.pathname}${window.location.search}`); }
export function isCatalogHistorySentinel(value: unknown): boolean { return typeof value === "object" && value !== null && Reflect.get(value, "entry") === "catalog-root"; }
export function installCatalogReplayNavigationGuard(): () => void { const navigate = Reflect.get(window, vinextNavigateKey); if (typeof navigate !== "function" || navigate === guardedCatalogReplayNavigation) return () => undefined; const guarded = (...args: readonly unknown[]) => { const navigationKind = args[2]; if (navigationKind === "traverse" && window.location.pathname === "/" && isCatalogHistorySentinel(window.history.state)) return Promise.resolve(); return Reflect.apply(navigate, window, args); }; guardedCatalogReplayNavigation = guarded; Reflect.set(window, vinextNavigateKey, guarded); return () => { if (Reflect.get(window, vinextNavigateKey) === guarded) Reflect.set(window, vinextNavigateKey, navigate); if (guardedCatalogReplayNavigation === guarded) guardedCatalogReplayNavigation = null; }; }
...
export function closeInAppPlaceDetail(): boolean { if (window.history.state?.entry !== "sheet") return false; const returnPath = readRestoreContext()?.returnPath ?? "/"; window.history.replaceState({ entry: "catalog-replay" }, "", returnPath); window.dispatchEvent(new CustomEvent(closeEvent)); return true; }
...
function source(snapshot: Snapshot): string { const value = snapshot.sourceUpdatedAt ?? snapshot.fetchedAt; const basis = snapshot.freshnessBasis === "fetched_at_degraded" ? "수집 시각 기준" : "원천 시각 기준"; return `${new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", dateStyle: "short", timeStyle: "short" }).format(new Date(value))} · ${basis}`; }
...
export function PlaceDetailClient({ areaCode, payload }: Readonly<{ areaCode: string; payload: DetailPayload }>) {
  ...
  function retryDetail(): void { window.location.reload(); }
  return <main className={styles.surface} data-detail-surface={surface} data-area-code={areaCode} aria-live="polite"><section className={panelClass} role={surface === "FULL_SCREEN" ? undefined : "dialog"} aria-modal={surface === "FULL_SCREEN" ? undefined : true} aria-label={`${payload.areaName ?? areaCode} 상세`} onKeyDown={surface === "FULL_SCREEN" ? undefined : handleDialogKeyDown}>
    ...
    <GlassPanel depth="floating" className={styles.surfaceHint}>원천 데이터와 선택 상태를 유지하는 공식 장소 상세입니다.</GlassPanel>
    {payload.status === "UNAVAILABLE" ? <section className={styles.section} data-detail-unavailable role="status" aria-live="polite"><div className={styles.sectionHeader}><h2>현재 데이터 연결 대기</h2><span className={styles.caption}>공식 장소 · {payload.areaName ?? areaCode}</span></div><p className={styles.notice} data-tone="warning">현재 혼잡도와 공식 예측을 확인할 수 없습니다. 장소 식별 정보와 카탈로그 경로는 유지합니다.</p><button className={styles.retry} type="button" onClick={retryDetail}>다시 시도</button></section> : null}
    ...
    <div className={styles.actions}><a className={styles.mapLink} href={`https://map.kakao.com/?q=${encodeURIComponent(payload.areaName ?? areaCode)}`} target="_blank" rel="noreferrer">카카오맵</a><a className={styles.mapLink} href={`https://map.naver.com/p/search/${encodeURIComponent(payload.areaName ?? areaCode)}`} target="_blank" rel="noreferrer">네이버지도</a><Link className={styles.back} href="/">목록으로 돌아가기</Link></div>
