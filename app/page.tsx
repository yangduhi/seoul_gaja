...
export async function generateMetadata({ searchParams }: HomeProps): Promise<Metadata> {
  const isPlaceNotFound = (await searchParams).placeNotFound === "1";
  return {
    title: "서울 가자 — 인파 레이더",
    description: "서울 공식 장소의 현재 혼잡도와 원천 기반 다음 시간을 살펴봅니다.",
    robots: isPlaceNotFound ? { index: false, follow: false } : undefined,
    other: {
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  const placeNotFound = visualQuery.placeNotFound === "1";
  ...
  if (result.status !== "READY") {
    ...
    return <CatalogSurface status="UNAVAILABLE" catalog={[]} snapshotStatus="UNAVAILABLE" sourceTime={null} recommendations={unavailableRecommendations} unavailableReason={result.reason} placeNotFound={placeNotFound} />;
  }
  ...
  return <CatalogSurface status="READY" catalog={catalog} snapshotStatus={result.data.snapshot.status} sourceTime={catalog[0]?.sourceUpdatedAt ?? catalog[0]?.fetchedAt ?? null} recommendations={recommendations} initialSelectedAreaCode={visualFixture !== null && visualQuery.visualState === "selected-detail" ? catalog[0]?.areaCode : undefined} placeNotFound={placeNotFound} />;
}
